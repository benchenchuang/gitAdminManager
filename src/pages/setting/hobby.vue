  ...
  <div class="panel">
    <Card :padding="22" :bordered="false" :dis-hover="true">
      <Form class="panel-form mb-20" inline :label-width="100">
        <FormItem label="兴趣爱好名称：">
          <Input v-model="params.label" placeholder="请输入名称"/>
        </FormItem>
    ...
    >
      ...
      >
        <Row>
          <Col span="17">
            <FormItem label="兴趣爱好" prop="label">
              <Input type="text" v-model="addparams.label" placeholder="兴趣爱好名称限制20字"/>
            </FormItem>
          </Col>
...
export default {
  ...
  methods: {
    ...
    getindustry() {
      settingApi.interest(this.params).then(res => {
    ...
    submit(name) {
      this.$refs[name].validate(valid => {
        if (valid) {
          ...
          settingApi.interest(this.addparams).then(res => {
    ...
    delCircle() {
      ...
      settingApi.interest(this.addparams).then(data => {
        this.getindustry();
