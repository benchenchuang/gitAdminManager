  ...
  <div class="panel">
    <Card :padding="22" :bordered="false" :dis-hover="true">
      <Form class="panel-form mb-20" inline :label-width="80">
        <FormItem label="举报名称">
          <Input v-model="params.label" placeholder="请输入举报名称"/>
        </FormItem>
    ...
    >
      ...
      >
        <Row>
          <Col span="17">
            <FormItem label="举报类型" prop="labelType">
              <Select  style="width:200px" v-model="addparams.labelType">
                <Option value="1">名片举报</Option>
                <Option value="2">作品举报</Option>
              </Select>
            </FormItem>
            <FormItem label="举报名称" prop="label">
    ...
    <Modal title="删除" v-model="isDel" :closable="false" @on-ok="delCircle">确定删除举报类型吗？</Modal>
  </div>
</template>
<script>
...
export default {
    ...
    return {
      columns: [
        ...
        },
        {
          title: "举报类型",
          render: (h, params) => {
              return h('div', {}, params.row.type==1 ? '名片' : '作品')
            }
        },
        {
          title: "举报名称",
      ...
      ruleValidate: {
        ...
        labelType:{required: true,message: "请选择举报类型"}
      },
      page: {
        totalNum: 0
      },
  ...
  methods: {
    ...
    getindustry() {
      settingApi.report(this.params).then(res => {
    ...
    submit(name) {
      this.$refs[name].validate(valid => {
        if (valid) {
          ...
          settingApi.report(this.addparams).then(res => {
    ...
    delCircle() {
      ...
      settingApi.report(this.addparams).then(data => {
        this.getindustry();
