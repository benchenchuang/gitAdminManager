<template>
  <div>
      ...
      <Card :padding="0" :bordered="false" :dis-hover="true">
        ...
        <div class="picks">
          ...
          <Select v-model="searchModal.reportTypeId" clearable @on-change="typeChange"
            ...
            <Option v-for="item in reportTypeList" :value="item.reportName" :key="item.reportName">{{ item.reportName
              }}
            </Option>
    ...
    <Edit v-on:modeed="modeed" v-model="isgoNextEd" :tDtaEd.sync="tDtaEd" v-if="callingEd"></Edit>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    methods: {
       ...
       */
      _reporttype() {
        reportApi.reportTypeList(1).then(data => {
          if (data.state == 0) {
